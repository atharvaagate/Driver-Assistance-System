...
def write_results(prediction, confidence, num_classes, nms_thresh = 0.4) :
    ...
    for ind in range(batch_size) :
        ...
        ##We use a Try- Except block for the cases when there are no detections at all.
        ## If we dont find detection, we skip the remaining loop
        try :
        ...
        for cl in img_classes :
            ## NMS
            ## Extract the detection for the class "cls"
            cls_mask = img_pred_*(img_pred_[:,-1] == cl).float().unsqueeze(1)
            class_mask_ind = torch.nonzero(cls_mask[:,-2]).squeeze()
            image_pred_class = img_pred_[class_mask_ind].view(-1, 7)  ##HEREEEEEEEEEE
            ...
            for i in range(idx) :
                ...
                ## Set 0 all the detections that iou< threshold
...
def bbox_iou(box1, box2) :
    ...
    ##co-ordinates of intersection rectangle
    ...
    i_area = torch.clamp(rect_x2 - rect_x1 + 1, min=0) * torch.clamp(rect_y2 - rect_y1 + 1, min=0)

    ## calculate the union area
    u_area = ( (b1_x2 - b1_x1 + 1)*(b1_y2 - b1_y1 + 1) ) + ( (b2_x2 - b2_x1 + 1)*(b2_y2 - b2_y1 + 1) )
    iou = i_area / (u_area - i_area)     #####HEREEEEEEEEEEEEEEEEEE
...
def unique(arr) :
    ...
    u_tensor = torch.from_numpy(unique_np)

    tensor = arr.new(u_tensor.shape)
    tensor.copy_(u_tensor)
...
def letterbox_image(img, dim) :
    ...
    temp = np.full((dim[1], dim[0], 3), 128)

    temp[(h-new_h)//2:(h-new_h)//2 + new_h,(w-new_w)//2:(w-new_w)//2 + new_w,  :] = img
